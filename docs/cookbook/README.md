...
| 11 | [Crypto-shredding and data classification](./11-crypto-shredding.md) | The `DataClassification` annotation, generated `Clone`/`View`/`LogValue` helpers, `pii_manifest.json` audit columns, `ForgetSubject`, `RewrapDEKs` for KEK rotation, redacted reads. |
...
| 14 | [cmdworkflow deployment](./14-cmdworkflow-deployment.md) | Production wiring of the workflow-orchestrated command bus. Leads with DBOS (the default per ADR 0033) on Postgres and on SQLite for local dev; Restate appears under *Alternative deployments* with topologies, three-step start, idempotency, observability, common pitfalls. |
