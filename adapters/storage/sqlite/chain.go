...
func (a *Adapter) BackfillEventHash(ctx context.Context, tenantID string, eventID uuid.UUID, hash, prevHash []byte) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: backfill event hash: %w", err)
	}
	if n == 0 {
		return fmt.Errorf("sqlite: backfill event hash: row tenant=%s event=%s already has a hash or does not exist", tenantID, eventID)
	}
	return nil
}
